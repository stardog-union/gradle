...
In prior versions of Gradle, it was possible to extend a configuration in different project.

Projects should not have knowledge of configurations in other projects.
This practice is discouraged because it can lead to unexpected behavior when configurations are extended in a way that is not intended by the configuration's owner.
